...
namespace GUI
{
    public partial class fQuanLyCachDung : Form
    {
        BUS_CachDung bus_loaiCachDung = new BUS_CachDung();
        DataTable dtLoaiCachDung;

        public fQuanLyCachDung()
        {
            InitializeComponent();
        ...
        private void grvDSCachDung_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0)
            {
                ...
                tbxMaLoaiCachDung.Text = row.Cells["Mã loại đơn vị"].Value.ToString();
                tbxTenLoaiCachDung.Text = row.Cells["Tên loại đơn vị"].Value.ToString();
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            {
                bool kq = bus_loaiCachDung.Delete(tbxMaLoaiCachDung.Text);
                if (kq)
                {
                    ReloadDSLoaiCachDung();
