...
namespace GUI
{
    public partial class fQuanLyThamSo : Form
    {
        BUS_ThamSo bus_thamSo = new BUS_ThamSo();
        DataTable dtThamSo;

        public fQuanLyThamSo()
        {
            InitializeComponent();
            ReloadBangThamSo();
        }
        void ReloadBangThamSo()
        {
            ...
            dtThamSo.Columns[0].ColumnName = "Mã tham số";
            dtThamSo.Columns[1].ColumnName = "Tên tham số";
            dtThamSo.Columns[2].ColumnName = "Trị giá";
            grvThamSo.DataSource = dtThamSo;
        ...
        private void grvThamSo_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex >= 0)
            {
                ...
                if (row.Cells["Mã tham số"].Value.ToString() == "")
                    return;

                tbxMaThamSo.Text = row.Cells["Mã tham số"].Value.ToString();
                tbxTenThamSo.Text = row.Cells["Tên tham số"].Value.ToString();
                tbxTriGia.Text = row.Cells["Trị giá"].Value.ToString();
